...
func network_decoder(from_network_channel <-chan Message) (<-chan struct{o Order; a string}, <-chan struct{s Orders; a string}, <-chan struct{v int; a string}, <-chan struct{v int; a string}) {
    order_from_network_channel := make(chan struct{o Order; a string});
    sync_from_network_channel := make(chan struct{s Orders; a string});
    ...
    go func() {
        for {
            ...
            case SYNC:
                data := struct{s Orders; a string}{s, addr};
